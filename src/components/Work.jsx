...
function Work({ workitems, setWorkitems, name, setName, date, currWeek, currmonth, }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const id = workitems.length ? workitems[workitems.length - 1].id + 1 : 1
    const newobj = { id, Name: name, checked: false }
    const newarr = [...workitems, newobj]
    setWorkitems(newarr)
    setName('')
  }

  const handleCheck = (id) => {
    const list = workitems.map((val) => val.id === id ? { ...val, checked: !val.checked } : val)
    setWorkitems(list)
  }

  const handleDelete = (id) => {
    const list = workitems.filter((val) => val.id !== id)
    setWorkitems(list)
  }
  ...
  return (
    <>
      <div className='taskimg text-center'>
        <h1 className='mb-4'>Work Task</h1>
        {
          (workitems.length) ?
            (

...
function House({ hitems, setHitems, name, setName, date, currWeek, currmonth }) {

  const handleSubmit = (e) => {
    e.preventDefault()
    const id = hitems.length ? hitems[hitems.length - 1].id + 1 : 1
    const newobj = { id, Name: name, checked: false }
    const newarr = [...hitems, newobj]
    setHitems(newarr)
    setName('')
  }

  const handleCheck = (id) => {
    const list = hitems.map((val) => val.id === id ? { ...val, checked: !val.checked } : val)
    setHitems(list)
  }

  const handleDelete = (id) => {
    const list = hitems.filter((val) => val.id !== id)
    setHitems(list)
  }
  ...
  return (
    <>
      <div className='taskimg text-center'>
        <h1 className='mb-4'>House Task</h1>
        {
          (hitems.length) ?
            (

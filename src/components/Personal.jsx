...
function Personal({ peritems, setPeritems, name, setName, date, currWeek, currmonth }) {

  const handleSubmit = (e) => {
    e.preventDefault()
    const id = peritems.length ? peritems[peritems.length - 1].id + 1 : 1
    const newobj = { id, Name: name, checked: false }
    const newarr = [...peritems, newobj]
    setPeritems(newarr)
    setName('')
  }

  const handleCheck = (id) => {
    const list = peritems.map((val) => val.id === id ? { ...val, checked: !val.checked } : val)
    setPeritems(list)
  }

  const handleDelete = (id) => {
    const list = peritems.filter((val) => val.id !== id)
    setPeritems(list)
  }
  ...
  return (
    <>
      <div className='taskimg text-center'>
        <h1 className='mb-4'>Personal Task</h1>
        {
          (peritems.length) ?
            (

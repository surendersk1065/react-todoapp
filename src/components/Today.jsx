...
function Container({ items, setItems, name, setName, date, currWeek, currmonth }) {
    ...
    const handleCheck = (id) => {
        const list = items.map((val) => val.id === id ? { ...val, checked: !val.checked } : val)
        setItems(list)
    }

    const handleDelete = (id) => {
        const list = items.filter((val) => val.id !== id)
        setItems(list)
    }
    ...
    return (
        <>
            <div className='taskimg text-center'>
                <h1 className='mb-4'>Today's Task</h1>
                {
                    (items.length) ?
                        (
